...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		alreadyDone, err := model.RechargeEpayWithAmount(verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.Money, c.ClientIP())
		if err != nil {
			switch {
			...
			case errors.Is(err, model.ErrTopUpAmountMismatch):
				logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 回调金额与订单不匹配 trade_no=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()))
